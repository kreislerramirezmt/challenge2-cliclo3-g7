...
function getAllUrlParams(url) {
  ...
  const _entries = _urlParams.entries();
  const _obj = {};
  for (const _entry of _entries) {
    var _paramName = _entry[0].trim();
    var _paramValue = (typeof (_entry[1]) === 'undefined' || (_entry[1].trim()) === "") ? true : decodeURIComponent(_entry[1].replace(/\+/g, ' ')).trim();
    _obj[_paramName] = _paramValue;
  }
  return _obj;
}
...
Navigo.prototype = {
  ...
  resolve: function resolve(current) {
    ...
    var GETParameters = getAllUrlParams(current || this._cLoc()) || extractGETParameters(current || this._cLoc());
